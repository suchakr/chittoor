...
` निर्विष्टविषयस्नेहः स दशान्तमुपेयिवान्।
 ...
 भुजविजितविमानरत्नाधिरूढः प्रतस्थे पुरीम्॥ १२-१०४ ***`.split(/\s*[\n\r][\n\r]\s*/g);
...
function renderSlokas (slokas) {
	...
	return `
		...
		<td> 
			<a href=${mp3}"> ${verse}</a> 
			<pre class="${cls}">${sloka}</pre>
			<audio controls="" ${mem?"loop":""} src=${mp3}> Your browser does not support the <code>audio</code> element. <a href=${mp3}> ${verse} </a> </audio>
		</td>
		</tr>
...
function setupSlokas() { 
	...
	document.getElementById("toggleMemorize").onclick = function() {
		Array.from(document.querySelectorAll('.tno-memorize')).forEach ( x => { x.hidden = !x.hidden })
	}
	renderSlokas(slokas)
}
